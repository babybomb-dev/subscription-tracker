...
import { renderFullCalendar, initFullCalendarControls } from './components/calendar.js';
...
function setupEventListeners() {
    ...
    document.getElementById('form-sub').addEventListener('submit', async (e) => {
        ...
        try {
            const hiddenId = document.getElementById('sub-id').value;
            const targetId = hiddenId || state.editingSubId;
            
            if (targetId) {
                await updateSubscription(targetId, data);
                showToast('อัปเดตเรียบร้อย', 'success');
            } else {
                await addSubscription(currentUser.uid, data);
                showToast('เพิ่มรายการใหม่เรียบร้อย!', 'success');
            }
            closeAllModals();
        } catch (error) {
