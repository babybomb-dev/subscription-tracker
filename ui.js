    ...
    subs.forEach(sub => {
        ...
        card.innerHTML = `
            ...
            <div class="space-y-1 relative z-10">
                ...
                ${sub.note ? `
                <div class="bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 text-[11px] px-2.5 py-1.5 rounded-lg flex items-start gap-1.5 mt-2 max-w-[90%]">
                    <i class="fa-solid fa-note-sticky mt-0.5 opacity-70 shrink-0"></i>
                    <span class="break-words line-clamp-2">${sub.note}</span>
                </div>` : ''}
            </div>
...
export function renderUpcomingList(subs, onPayClick, onQrClick, exchangeRates = {}) {
    ...
    upcomingSubs.forEach(sub => {
        ...
        let currencyDisplay = sub.currency ? sub.currency.toUpperCase() : 'THB';
        if (currencyDisplay !== 'THB' && currencyDisplay !== 'thb') {
            const code = currencyDisplay.toLowerCase();
            const rate = exchangeRates[code] || 1;
            const thbEquivalent = (parseFloat(sub.price) / rate).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
